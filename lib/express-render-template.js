...
var cheerio = require('cheerio'),           // used to convert raw html into jQuery style object to we can use css selectors
    ...
    cache = new NodeCache({ stdTTL: 1800 }); // cache used to store html by url - auto clears after 30 minutes 
...
module.exports = function(req, res, next){
    ...
    res.render = function(view, model, callback){
        ...
        if (!model.template){
            ...
            var templateSelector = model.templateSelector || 'body';
            
            // resolve the template, either url to jquery object or html
            resolveTemplate(model.template).then(function($template){
                ...
                res.render(view, model, function(err, html) {
                    ...
                    if ($viewBody && $viewBody.length>0){
                        ...
                        $template(templateSelector).html($viewBody.html());    
                    }
                    else{
                        // no body tag in view, insert all content
                        $template(templateSelector).html($view.html());   
                    }
